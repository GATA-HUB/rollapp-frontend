...
task("create-free-nft-pool", "Creates a staking pool for Free NFT contract")
  .setAction(async (taskArgs: TaskArgs, hre: HardhatRuntimeEnvironment) => {
    try {
      // Read deployment files
      const nftDeployment = JSON.parse(
        readFileSync(join(process.cwd(), "free-nft-deployment.json"), "utf8")
      );
      ...
      console.log("Creating pool for Free NFT collection...");
      ...
      const rewardPerSecond1 = "2000000000000000000"; // 2 tokens per second (higher reward for free NFT)
      ...
      const poolInfo = {
        ...
        poolIndex: 1, // Second pool (first was index 0)
        startTime,
        endTime,
        rewardPerSecond: rewardPerSecond1,
        note: "Higher reward rate for free NFT collection"
      };
      
      writeFileSync(
        join(process.cwd(), "free-nft-pool.json"),
        JSON.stringify(poolInfo, null, 2)
      );

      console.log("Pool created successfully!");
      console.log("Pool details saved to free-nft-pool.json");
